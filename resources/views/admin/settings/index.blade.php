...
@section('content')
    ...
    @if ($isSuper && ! $brandingIsHolding)
        <div class="notice ctx"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 11v5M12 8h.01"/></svg><span>Vous modifiez les paramètres de la filiale <strong>{{ $brandingFilialeName }}</strong>. Types d'événement, comptes et branding affichés ci-dessous sont ceux de cette filiale.</span></div>
    @endif
    ...
    <div class="tabs" role="tablist" aria-label="Sections des paramètres">
        ...
        <button class="tab" role="tab" id="tab-comptes" aria-selected="false" onclick="Settings.tab('comptes')">Comptes</button>
    ...
    <section class="panel" id="panel-comptes" role="tabpanel" hidden>
        <div class="card">
            <div class="card__hd">
                <div>
                    <h2>{{ $isSuper ? 'Comptes de la holding' : 'Comptes de ma filiale' }}</h2>
                    <p>{{ $isSuper ? 'Vue transversale : le super administrateur gère les comptes de toutes les filiales.' : 'Membres autorisés à accéder au tableau de bord.' }}</p>
                </div>
            ...
            <div class="tscroll">
                <table class="dt" aria-label="Comptes">
                    <thead><tr><th>Membre</th>@if($isSuper)<th>Filiale</th>@endif<th>Rôle</th><th>Statut</th><th>Dernière connexion</th><th class="r-actions">Actions</th></tr></thead>
    ...
    <section class="panel" id="panel-general" role="tabpanel" hidden>
        <form class="card" method="POST" action="{{ route('admin.settings.branding') }}" enctype="multipart/form-data">
            @csrf
            <div class="card__hd"><div><h2>Général &amp; branding</h2><p>{{ $brandingIsHolding ? 'Identité de la holding — appliquée par défaut à toutes les filiales qui héritent du branding.' : 'Identité de la filiale ' . $brandingFilialeName . ', affichée sur ses écrans, e-mails et pages publiques.' }}</p></div></div>
            <div class="card__body">
                @unless ($brandingIsHolding)
                    <label class="check" style="display:flex;align-items:center;gap:9px;margin-bottom:16px">
                        <input type="checkbox" name="brand_inherit" value="1" id="brandInherit" {{ $brandInherits ? 'checked' : '' }} onchange="Settings.toggleBrandInherit(this.checked)">
                        <span>Hériter du branding de la holding (ACS Groupe)</span>
                    </label>
                @endunless
                <div id="brandFields" @if ($brandInherits) style="display:none" @endif>
                    <div class="field">
                    ...
                    <div class="grid2" style="margin-top:14px">
                        ...
                        @else
                            <div class="help" style="align-self:end">Fuseau horaire et format de date sont réglés au niveau de la holding et s'appliquent à toutes les filiales.</div>
                        @endif
                    </div>
                </div>
            </div>
